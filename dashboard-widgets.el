...
(declare-function all-the-icons-octicon "ext:data-faicons.el")
...
(defun dashboard-insert-heading (heading &optional shortcut)
  ...
  (when (and (display-graphic-p)
    ...
    (unless (or (fboundp 'all-the-icons-faicon)
                (require 'all-the-icons nil 'noerror))
      (error "Package `all-the-icons' isn't installed"))

    (insert (cond
             ((string-equal heading "Recent Files:")
              (all-the-icons-faicon (cdr (assoc 'recents dashboard-heading-icons))
                                     :height 1.2 :v-adjust 0.0 :face 'dashboard-heading))
             ((string-equal heading "Bookmarks:")
              (all-the-icons-faicon (cdr (assoc 'bookmarks dashboard-heading-icons))
                                     :height 1.2 :v-adjust 0.0 :face 'dashboard-heading))
             ((or (string-equal heading "Agenda for today:")
                  (string-equal heading "Agenda for the coming week:"))
              (all-the-icons-faicon (cdr (assoc 'agenda dashboard-heading-icons))
                                     :height 1.2 :v-adjust 0.0 :face 'dashboard-heading))
             ((string-equal heading "Registers:")
              (all-the-icons-faicon (cdr (assoc 'registers dashboard-heading-icons))
                                     :height 1.2 :v-adjust 0.0 :face 'dashboard-heading))
             ((string-equal heading "Projects:")
              (all-the-icons-faicon (cdr (assoc 'projects dashboard-heading-icons))
                                     :height 1.2 :v-adjust 0.0 :face 'dashboard-heading))
             (t " ")))
    (insert " "))
                   ...
                   (icon (if (and (not (file-remote-p path))
                           ...
                           (cond
                            ((or (string-equal ,section-name "Agenda for today:")
                             ...
                             (all-the-icons-faicon "circle-o" :height 1.0 :v-adjust 0.01))
                            ((file-remote-p path)
                             (all-the-icons-faicon "map-o" :height 1.0 :v-adjust 0.01))
